...
class Dammit:
    ...
    def __init__(
        ...
        self.representative_sequences = set(
            [a.lstrip(">") for a in representative_sequences]
        )
    ...
    def postprocessing(self):
        ...
        self.clusters_and_sequences_per_gene = self.combine_dicts(
            clusters_per_gene, sequences_per_gene
        )
    ...
    def calculate_lost_genes(self, gene_dict_before, gene_dict_after):
        ...
        lost_genes = genes_detected_before_clustering.difference(
            genes_detected_after_clustering
        )
    ...
    def removed_sequences_per_gene(self, gene_dict_before, gene_dict_after):
        ...
        removed_per_cluster = []
        for (key_b, value_b), (key_a, value_a) in zip(
            gene_dict_before.items(), gene_dict_after.items()
        ):
            b = set(value_b)
            a = set(value_a)
        ...
        removed_sequences["mean"] = np.sum(removed_per_cluster) / len(
            gene_dict_before.keys()
        )
    ...
    def gene_dict(self, gff, namemap, suffix):
        ...
        names = (
            annotations.sort_values(by=["seqid", "score"], ascending=True)
            .query("score < 1e-05")
            .drop_duplicates(subset="seqid")[["seqid", "Name"]]
        )
        ...
        new_file.to_csv(
            f"{self.output_dir}/genes_{suffix}.csv",
            sep="\t",
            columns=["transcript_id", "Name"],
        )
