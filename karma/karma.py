...
def create_lookup_dict(clusters_with_subcluster, sequences):
    ...
    for cl_no, cluster in enumerate(clusters_with_subcluster, 1):
        for mcl_cluster in cluster:
            mcl_subclusters[index] = {
                "previous_cluster": cl_no,
                "mcl_subcluster": mcl_cluster,
            }
            index += 1

    # Check if dictionary is okay.
    length_of_dict = sum(
        [len(subcluster["mcl_subcluster"]) for subcluster in mcl_subclusters.values()]
    )
    logger.debug(f"length of dict: {length_of_dict}, {len(sequences)}")
    assert length_of_dict == len(
        sequences
    ), "The creation of the lookup dictionary went wrong."
...
def combine_connected_subclusters(mcl_subclusters, mcl_groups_to_combine):
    ...
    assert len(list(nx.k_edge_subgraphs(mcl_cluster_connection_graph, k=1))) == len(
        connected_subclusters
    ), "Combining went wrong"
...
def main():
    ...
    assert len(flatten(labeled_contigs) + flatten(unlabeled_contigs)) == len(
        sequences
    ), "LOST A SEQUENCE :("
    ...
    for cluster in labeled_contigs:
        ...
        assert len(cluster_graph.nodes()) == len(
            cluster
        ), "Cluster graph not equal the cluster group"

        # Draw
        # logger.info(f"NODES: {cluster_graph.nodes}")
        cluster_graph.draw_graph(
            f"{graph_visual_dir}/cluster_{i}_A.svg", draw=args.DRAW
        )
    ...
    assert len(flatten(trimmed_clusters) + flatten(unlabeled_contigs)) == len(
        sequences
    ), "LOST A SEQUENCE :("
    ...
    for i, cluster in enumerate(trimmed_clusters, 1):
        ...
        # logger.info(f"NODES: {cluster_graph.nodes}")
        cluster_graph.draw_graph(
            f"{graph_visual_dir}/cluster_{i}_B.svg", draw=args.DRAW
        )
        ...
        non_mcl_cluster_contigs = cluster_graph.get_contigs_not_in_mcl_cluster_stdout(
            mcl_result, original_contigs=cluster
        )

        assert (
            len(flatten(cluster_graph.mcl_cluster)) != 0
        ), f"Cluster {i} has no mcl clusters:\n {cluster}"
        cluster_graph.remove_nodes_from(non_mcl_cluster_contigs)
        assert len(flatten(non_mcl_cluster_contigs)) + len(cluster_graph.nodes) == len(
            cluster_with_unlabeled
        )
        ...
        # logger.info(f"NODES: {cluster_graph.nodes}")
        cluster_graph.draw_graph(
            f"{graph_visual_dir}/cluster_{i}_C.svg", draw=args.DRAW
        )
    ...
    assert len(flatten(clusters_with_subcluster)) + len(
        flatten(unlabeled_contigs)
    ) == len(sequences)
    ...
    # DRAW
    cluster_graph.draw_graph(
        f"{graph_visual_dir}/cluster_unlabeled.svg", draw=args.DRAW
    )

    mcl = Mcl(threads=args.THREADS, inflation=args.INFLATION)
    mcl_result = mcl.run_pipe(graph_file=cluster_graph.edge_list())

    cluster_graph.get_contigs_not_in_mcl_cluster_stdout(
        mcl_result, original_contigs=unlabeled_contigs
    )
    ...
    clustered_sequence_names = (
        unlabeled_representative_sequences + cluster_representative_sequences
    )
    ...
    if args.REARRANGE:
        ...
        mcl_groups_to_combine = calc_connections_between_mcl_subclusters(
            mcl_subclusters, weight_cutoff=args.THRESHOLD
        )
        ...
        new_cluster_subcluster += combine_connected_subclusters(
            mcl_subclusters, mcl_groups_to_combine
        )

        # Remove already added clusters from lookup dict
        mcl_subclusters = remove_already_added_clusters(
            from_dict=mcl_subclusters, remove=set(flatten(mcl_groups_to_combine))
        )
        ...
        assert len(flatten(new_cluster_subcluster)) == len(
            sequences
        ), "rearraning groups went wrong."
    else:
    ...
    if args.ANNOTATE:
        ...
        dammit = Dammit(
            first_file=args.FASTA_FILE,
            second_file=fasta_output_file,
            output_dir=dammit_dir,
            kmer_clusters=labels_after_kmer_clustering,
            karma_clusters=labels_after_read_graph,
            representative_sequences=clustered_sequence_names,
            threads=args.THREADS,
        )
        dammit.update_database(
            database_dir=args.DATABASE_DIR, busco_group=args.BUSCO_GROUP
        )
        dammit.run()
